...
function MainPage() {
  ...
  const products = useSelector((state) => state.products.productSlice);
  const loadStatus = useSelector((state) => state.products.loadStatus);
      ...
      <content className="content">
        <div className="top_brand">
          ...
          <div className="top_brand_title">
            <img src={top_brand_title} width="12" height="93" />
        ...
        <div className="products center">
          ...
          <div >
            {loadStatus === "loading" && console.log('Данные загружаются...')}
            {loadStatus === "failed" && console.log('Ошибка загрузки данных')}
            {loadStatus === "success" && (
          ...
          <div className="products_button">
            <Link className="browse_button" to="/internet-shop_react/catalog">
              Browse All Product
            </Link>
          </div>
        </div>
